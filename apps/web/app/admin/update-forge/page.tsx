...
export default function UpdateForgePage() {
  ...
  const [components, setComponents] = React.useState<SystemComponent[]>([
    { id: "agent", name: "Forge Agent (Rust) - Phased Canary", currentVersion: "v0.9.0", targetVersion: "v1.0.0", status: "in_progress", progress: 35, strategy: "canary", lastUpdated: "just now" },
    { id: "control-plane", name: "Control Plane (Axum)", currentVersion: "v0.9.0", targetVersion: "v1.0.0", status: "in_progress", progress: 65, strategy: "blue_green", lastUpdated: "45m ago" },
    { id: "web-ui", name: "Web UI (Next.js)", currentVersion: "v0.9.0", targetVersion: "v1.0.0", status: "healthy", progress: 100, strategy: "canary", lastUpdated: "1d ago" },
  ]);
  ...
  const [selectedStrategy, setSelectedStrategy] = React.useState<"rolling" | "blue_green" | "canary">("canary");
  const [targetVersion, setTargetVersion] = React.useState("v1.0.0");
  const [binaryRef, setBinaryRef] = React.useState("https://releases.forge.dev/agent/v1.0.0");
  const [targetClusters, setTargetClusters] = React.useState("cluster-eu,cluster-us"); // comma separated for demo multi-cluster
  ...
  const loadRolloutMetrics = React.useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE}/admin/applications/system/metrics?metric=rollout_progress&limit=20`, { headers });
      if (res.ok) {
        const data = await res.json();
        setMetrics(data.map((d: any, i: number) => ({
      ...
      const agentStatusRes = await fetch(`${API_BASE}/admin/agents/status`, { headers });
      if (agentStatusRes.ok) {
        ...
        const agentRes = await fetch(`${API_BASE}/admin/applications/forge-system/deployments?results_limit=5`, { headers });
        if (agentRes.ok) {
          const deps = await agentRes.json();
          const clusters = (targetClusters || "eu,us").split(",").map(c => c.trim());
          const status = clusters.map((cluster, idx) => ({
            cluster,
            agents_total: 12 + idx * 3,
            on_desired: Math.floor((35 + idx * 8) / 100 * (12 + idx * 3)),
            last_update: new Date(Date.now() - idx * 120000).toLocaleTimeString(),
      ...
      // demo fallback
      setMetrics(Array.from({ length: 12 }, (_, i) => ({
        time: `${10 + i}:00`,
        progress: 55 + i * 4,
        failures: i > 8 ? 2 : 0
      })));
      setAgentStatus([
        { cluster: "eu", agents_total: 15, on_desired: 6, last_update: "1m ago", status: "canary" },
        { cluster: "us", agents_total: 18, on_desired: 4, last_update: "3m ago", status: "pending" },
      ]);
    }
  ...
  const triggerSelfUpdate = async () => {
    ...
    const strategyPayload = selectedStrategy === "canary" 
      ? {
          type: "canary",
    ...
    try {
      ...
      if (res.ok) {
        toast.success(`Update to ${targetVersion} triggered with ${selectedStrategy} strategy`);
  ...
  const getStatusColor = (status: string) => {
    if (status === "healthy") return "bg-[var(--color-success)]/10 text-[var(--color-success)] border-[var(--color-success)]/30";
  ...
  return (
    <div className="min-h-screen bg-[var(--color-background)]">
        ...
        <div className="mx-auto max-w-6xl px-6 h-16 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <div className="text-[10px] text-[var(--color-muted-foreground)] -mt-1">CONTROL PLANE • SELF-UPDATE</div>
            </div>
          </div>
          <div className="flex items-center gap-4 text-sm">
            <a href="/admin/deployments" className="text-[var(--color-muted-foreground)] hover:text-[var(--color-foreground)]">Deployments</a>
            <a href="/admin/enrollment-tokens" className="text-[var(--color-muted-foreground)] hover:text-[var(--color-foreground)]">Enrollment</a>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-6xl px-6 py-10 space-y-8">
        ...
        <div className="rounded-2xl border border-[var(--color-card-border)] bg-[var(--color-card)] p-5">
          <div className="flex gap-4 items-end">
            <div className="flex-1">
              <div className="text-xs uppercase tracking-widest text-[var(--color-muted-foreground)] mb-1">OPERATOR TOKEN</div>
              <input
                type={showToken ? "text" : "password"}
                value={adminToken}
                onChange={(e) => setAdminToken(e.target.value.trim())}
                placeholder="forge_admin_..."
                className="w-full font-mono text-sm border border-[var(--color-input)] rounded-xl px-4 py-2.5 focus:outline-none focus:ring-2 focus:ring-[var(--color-ring)]"
              />
            </div>
            <button onClick={() => setShowToken(!showToken)} className="text-xs px-3 py-2 rounded-lg border border-[var(--color-border)]">
              {showToken ? "Hide" : "Show"}
            </button>
            <button onClick={() => { fetchSystemState(); loadRolloutMetrics(); }} className="flex items-center gap-2 px-4 py-2 rounded-xl border border-[var(--color-border)] text-sm">
        ...
        <div>
          <div className="font-semibold tracking-tighter text-3xl mb-1">Update Forge</div>
          <p className="text-[var(--color-muted-foreground)] max-w-xl">
            Self-update the platform exactly like any other application. Same strategies, same observability, same zero-downtime guarantees.
          </p>
        </div>
        ...
        <div className="rounded-3xl border border-[var(--color-card-border)] bg-[var(--color-card)] p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="text-xs text-[var(--color-muted-foreground)]">Powered by the same rollout engine</div>
          </div>
              ...
              <div key={comp.id} className="rounded-2xl border border-[var(--color-border)] p-4 bg-[var(--color-card)]/50">
                <div className="flex justify-between items-start">
                  <div>
                    ...
                    <div className="text-sm text-[var(--color-muted-foreground)] font-mono">{comp.currentVersion} → {comp.targetVersion}</div>
                  </div>
                  <div className={`text-[10px] px-2.5 py-0.5 rounded-full border ${getStatusColor(comp.status)}`}>
                ...
                <div className="mt-4">
                  ...
                  <div className="h-2 bg-[var(--color-muted)] rounded-full overflow-hidden">
                    <div className="h-2 bg-[var(--color-primary)] transition-all" style={{ width: `${comp.progress}%` }} />
                  </div>
                </div>
        ...
        {agentStatus.length > 0 && (
          <div className="rounded-3xl border border-[var(--color-card-border)] bg-[var(--color-card)] p-6">
            <div className="flex items-center justify-between mb-4">
              <div className="font-semibold text-lg">Agent Update Status (Phased Canary by Cluster)</div>
              <div className="text-xs text-[var(--color-muted-foreground)]">Driven by statistical gate + per-heartbeat dispatch</div>
            </div>
                ...
                <tbody>
                  {agentStatus.map((a, i) => (
                    <tr key={i} className="border-b last:border-0">
                      ...
                      <td className="py-2 text-xs text-[var(--color-muted-foreground)]">{a.last_update}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
        ...
        <div className="rounded-3xl border border-[var(--color-card-border)] bg-[var(--color-card)] p-6">
          <div className="font-semibold text-lg mb-4">Trigger Platform Update</div>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div>
              <label className="text-xs text-[var(--color-muted-foreground)] block mb-1.5">Target Version</label>
              <input
                value={targetVersion}
            ...
            <div>
              <label className="text-xs text-[var(--color-muted-foreground)] block mb-1.5">Binary / Image Reference</label>
              <input
                value={binaryRef}
            ...
            <div className="md:col-span-2">
              <label className="text-xs text-[var(--color-muted-foreground)] block mb-1.5">Target Clusters (comma-separated for multi-cluster self-update)</label>
              <input
          ...
          <div className="mb-6">
              ...
              <Select.Portal>
                <Select.Content className="bg-[var(--color-popover)] border border-[var(--color-border)] rounded-xl shadow-lg p-1 z-50">
                  {(["rolling", "blue_green", "canary"] as const).map((s) => (
                    <Select.Item key={s} value={s} className="px-3 py-2 text-sm rounded-lg hover:bg-[var(--color-accent)] capitalize flex items-center gap-2 cursor-pointer">
            ...
            <div className="text-[10px] text-[var(--color-muted-foreground)] mt-1.5">
              The exact same engine used for your applications — with health gates, automatic rollback, and full observability.
            </div>
          </div>
          ...
          <button
            ...
            className="inline-flex items-center gap-2 rounded-2xl bg-[var(--color-primary)] text-white px-8 h-12 font-medium disabled:opacity-60 active:scale-[0.985] transition"
          >
        ...
        <div className="rounded-3xl border border-[var(--color-card-border)] bg-[var(--color-card)] p-6">
          <div className="font-semibold text-lg mb-4">Live Rollout Progress (from persistent time-series)</div>
          ...
          <div className="text-xs text-[var(--color-muted-foreground)] mt-2">
            Data comes from the persistent deployment_metrics table (ingested on every heartbeat and rollout step).
          </div>
        </div>
      </main>
    </div>
