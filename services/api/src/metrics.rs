...
impl ControlPlaneMetrics {
    pub fn new() -> Self {
        ...
        let deployments_by_status = GaugeVec::new(
            Opts::new("forge_deployments_by_status", "Deployments grouped by current status"),
            &["status"]
        ).unwrap();
        ...
        let agents_total = Counter::new(
            "forge_agents_total",
            "Total agents ever enrolled"
        ).unwrap();
        ...
        let job_results_total = CounterVec::new(
            Opts::new("forge_job_results_total", "Job results received from agents"),
            &["job_type", "success"]
        ).unwrap();
        ...
        let container_health_gauge = GaugeVec::new(
            Opts::new("forge_container_health", "Per-container health indicators from HealthCheck jobs (1=healthy)"),
        ...
        registry.register(Box::new(deployments_by_status.clone())).unwrap();
        registry.register(Box::new(agents_connected.clone())).unwrap();
        registry.register(Box::new(agents_total.clone())).unwrap();
        registry.register(Box::new(jobs_dispatched_total.clone())).unwrap();
        registry.register(Box::new(job_results_total.clone())).unwrap();
        registry.register(Box::new(job_duration_seconds.clone())).unwrap();
        registry.register(Box::new(healthchecks_total.clone())).unwrap();
        registry.register(Box::new(container_health_gauge.clone())).unwrap();
        ...
        registry.register(Box::new(rollout_progress_percent.clone())).unwrap();
        registry.register(Box::new(rollout_failure_count.clone())).unwrap();
